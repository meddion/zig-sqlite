describe 'database' do
  ...
  def run_script(commands)
    raw_output = nil
    IO.popen('./zig-out/bin/sqlite test.db', 'r+') do |pipe|
      begin
        # Send commands to the program
        commands.each do |command|
          pipe.puts(command)
          pipe.flush # Ensure the command is sent immediately
        end
        # pipe.puts(commands.join("\n") + "\n")
      rescue Errno::EPIPE
        # Handle broken pipe gracefully
        puts 'The pipe was closed prematurely by the program.'
      ensure
        pipe.close_write # Close the writing end
      end

      # Read entire output
      raw_output = pipe.read unless pipe.closed?
    end
    raw_output.nil? ? [] : raw_output.split("\n")
  end
